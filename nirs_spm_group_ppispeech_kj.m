function [gavg_beta, group_beta, xX, index_group, mask, brain_view, fname_interp_cov] = nirs_spm_group_ppispeech_kj(fname_ginterp_betas, min_subj)
...
for kk = 2:nsubj
    ...
    indiv_beta = zeros(nbeta, nvox_brain);
    %kajaedit
    [aaa,bbb]=size(interp_beta);
    if aaa==15
        interp_beta_temp=[interp_beta(1:4,:);interp_beta(12:15,:)];
        interp_beta=interp_beta_temp;
    end
    indiv_beta(:,index_mask) = interp_beta;
