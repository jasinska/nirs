function [SPM_nirs] = specification_batch_PPI(fname_nirs, hb, HPF, LPF, method_cor, dir_save, flag_window, hrf_type, units,  names, onsets, durations, PPInirs)
...
elseif nargin == 10 % multiple condition
    ...
    for kk = 1:size(names, 2)
        ...
        SPM.Sess.C.C(:,1)=PPInirs.ppi;
        SPM.Sess.C.C(:,2)=PPInirs.Y;
        SPM.Sess.C.C(:,3)=PPInirs.P;
 
        SPM.Sess.C.name{1,1} = 'ppi';
        SPM.Sess.C.name{1,2} = 'ppiY';
        SPM.Sess.C.name{1,3} = 'ppiP';
elseif nargin == 13
    if units == 0
        SPM.xBF.UNITS = 'scans';
    elseif units == 1
